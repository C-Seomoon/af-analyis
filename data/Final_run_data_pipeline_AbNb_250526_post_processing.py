...
OUTPUT_DIR = 'pipeline_results_AbNb'    
...
logging.basicConfig(
    ...
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(f"{output_dir}/process_{query_name}.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger(__name__)
...
def main():
    """메인 실행 함수"""
    # 특정 쿼리만 처리하도록 수정
    queries = ['8fgx_8fgx', '8hp9_8hp9']
    
    logger.info(f"Processing {len(queries)} specific queries: {queries}")
