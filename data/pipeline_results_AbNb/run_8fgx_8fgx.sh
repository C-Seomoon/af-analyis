#!/bin/bash
#SBATCH --job-name=af_8fgx_8fgx
#SBATCH --output=pipeline_results_AbNb/slurm_8fgx_8fgx_%j.log
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"pipeline_results_AbNb/process_8fgx_8fgx.log"),
...
def calculate_all_metrics_per_query_optimized(query_group_data, **kwargs):
    ...
    try:
        ...
        logger.info(f"✨ Query {query_name} completed in {total_time:.2f}s")
        
        return query_name, temp_data.df
        
    except Exception as e:
        total_time = time.time() - total_start
        logger.error(f"❌ Error processing query {query_name} after {total_time:.2f}s: {str(e)}")
        import traceback
...
def process_query(query_name):
    base_path = "/home/cseomoon/project/ABAG/DB/AbNb_structure/AF3/native"
    ...
    try:
        ...
        for _, group_df in df.groupby("query"):
            ...
            if result_df is not None:
                output_file = os.path.join("pipeline_results_AbNb", f"metrics_{name}.csv")
